...
parser.add_argument('--dim', type=int, default=160, help='dimension of user and entity embeddings')
parser.add_argument('--n_iter', type=int, default=2, help='number of iterations when computing entity representation')
parser.add_argument('--l2_weight', type=float, default=1e-4, help='weight of l2 regularization')
parser.add_argument('--lr', type=float, default=5e-4, help='learning rate')
parser.add_argument('--ratio', type=float, default=0.8, help='size of training dataset')
parser.add_argument('--n_topo_feats', type=int, default=160, help='dim of topology features')
...
kg = read_kg('data/drugbank/train2id_refined.txt')
...
def update_E(net_E, drug_entity_list, feat_mat, batch_edges, labels, D_t2a, D_a2t, loss, trainer_E, device):
    ...
    y_pred, gcn_out, t2a_mtx, a2t_mtx,_,_ = net_E(drug_entity_list, feat_mat, batch_edges)
...
def update_D_t2a(net_E, drug_entity_list, feat_mat, batch_edges, D_t2a, loss, trainer_D_t2a, device):
    ...
    mone = (one * -1).to(device)
    net_E.eval()
    _, _, t2a_mtx, _,_,_ = net_E(drug_entity_list, feat_mat, batch_edges)
...
def update_D_a2t(net_E, drug_entity_list, feat_mat, batch_edges, D_a2t, loss, trainer_D_a2t, device):
    ...
    mone = (one * -1).to(device)
    net_E.eval()
    _, gcn_out, _, a2t_mtx,_,_ = net_E(drug_entity_list, feat_mat, batch_edges)
...
for k,(train_index,test_index) in enumerate(skf.split(new_example_data,new_example_label)):
    ...
    for epoch in range(n_epochs):
        ...
        with torch.no_grad():
            ...
            for i, (edges, labels) in enumerate(vaild_loader):
                edges, labels = edges.to(device), labels.to(device)
                y_pred, _, _, _,_,_= model(drug_list, feat_mat, edges, False)
                y_pred = y_pred.reshape(-1)
                labels = labels.to(torch.float32)
            ...
            if vaild_loss < best_vaild_loss:
                ...
                counter = 0
                save_file = "checkpoint/drugbank_binary_emb160_{}.pth".format(k)
    ...
    # Testing phase
    with torch.no_grad():
        model.load_state_dict(torch.load('checkpoint/drugbank_binary_emb160_{}.pth'.format(k)))
        model.eval()
...
pd.DataFrame(results,columns=['fold','acc','aupr','auroc','f1']).to_csv('drug_cross_validation.csv')
